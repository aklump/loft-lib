...
class Dates {
   ...
   * Dates constructor.
   *
   * @param string $localTimeZoneName
   *   The name of the timezone to use for local times, this is used when the
   *   timezone is not specified in dates used by this class.  Do not use PDT
   *   or PST, or the like as they have different behavior across PHP versions.
   *    Better to use America/Los_Angeles.
   * @param string $nowString
   *   Optional.  This string will be used to compute the current moment in
   *   time. By default the string is 'now'.  It is also used for how dates are
   ...
   *   The bounds control how things like 'monthly' gets normalized. By default
   *   the bounds are 1 month beginning the 1st of the current month.
   * @param \DateInterval|null $periodInterval
   *   If you want a normalized monthly to generate 12 dates instead of 1, you
   *   would set this to 'P1Y' and set the $periodStart to the earliest month
   *   of the year, and probably January first.
   * @param array $defaultTime
   *   A three element indexed array with hour, minute, second for the default
   *   UTC time when using the normalize() method. Be careful here because if
