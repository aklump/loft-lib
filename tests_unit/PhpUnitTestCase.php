...
class PhpUnitTestCase extends TestCase {
   ...
   *
   * @param string $method
   * @param array $args An array of arguments, the elements of which must be
   *   references (see code example).
   *
   * @return mixed
   *
   * @code
   ...
   * @return mixed
   *
   * Often you will create a Mocked dependency of $this->obj, this following
   *   code shows how this is done, and then reinjected into $this->obj.
   * @code
   *   // Replace one of Money dependencies with a mocked $db.
   *   $this->objArgs[1] = $this->getMockable('fetchAll', [
   *      // History
   *      ['target_key' => 'value', 'target_id' => 'checking', 'value' =>
   *   "500"],
   *   ]);
   *
   *   // Now refresh $this->obj with the new db Mock.
   *   $this->createObj();
   *   ... the $db object in Money is a mocked db with the method fetchAll
   *   ready to return our history array.
   * @endcode
   */
  ...
  /**
   * Destroy then create anew a sandbox directory for temporary file test
   * operations.
   *
   * Put this in your setUp() method usually and use $this->sb as directory to
   * the sandbox.
   *
   * @return $this
   */
